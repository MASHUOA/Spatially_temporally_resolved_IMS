---
title: "Manuscript R markdown file"
author: "Dingchang Shi"
date: "2023-11-07"
...
match_mz = function(x, ref, tol = 1e-10,unit=c("Da","ppm"))
...
post_mod<- function (fig, point_size){for (n in 1: length(fig[["x"]][["data"]])){
...
if(!require("correally")) devtools::install_github("addiversitas/correally",upgrade = "never", quiet = T)
...
annotate_peak<-function(deconv_peaklist,
                        wd=getwd(),
                        adducts=c("M-H","M+Cl"),
                        ppm=5,
                        candidate_list=NULL,
                        ...
                        isotope_sub=NULL,
                        BPPARAM=bpparam(),
                        workdir_hmdb="C:/Users/admgguo484/Documents/keggdb/",
...
threshold=0.00
  ...
  Quant_list="C:\\Users\\admgguo484\\Documents\\omics software\\maldiimaging\\Maldi_imaging - Copy\\DB\\SMPdb.csv"
  cal.mz=F
  mzlist_bypass=T
  PMF_analysis=TRUE
  ...
  cal.mz = T
   bypass=F
 if (attachHMDB){
  Meta_feature_list<-Meta_feature_list_fun(workdir=workdir_hmdb,
                                           database = "SMPdb.csv",
                                           adducts=adducts,
                                           cal.mz = T,
                                           atom_isotope_sub=atom_isotope_sub,
                                           bypass=F)
  
  
 }else{
    Meta_feature_list<-NULL
  }

if(!is.null(candidate_list)){
  required_col=c("mz","Metabolite.Name","adduct","formula","charge","mass")
  candidate_list=data_test_rename(required_col,candidate_list) 
  if (is.null(candidate_list$Isotype)) candidates$Isotype="Normal"
  Meta_feature_list<-rbind(candidate_list,Meta_feature_list)
}

if (!is.null(isotope_sub)){
  
}

Meta_feature_list$formula=paste(Meta_feature_list$formula,Meta_feature_list$Isotype,sep="@")
aggregate<-Meta_feature_list[,c("Metabolite.Name","formula")]
aggregate<-aggregate %>%  group_by(formula) %>% summarize(Metabolite.Name=paste(Metabolite.Name,  collapse = ";"))
Meta_feature_list$Metabolite.Name<-NULL
aggregate<-merge(aggregate,unique(Meta_feature_list[,colnames(Meta_feature_list)!="Metabolite.Name"]),by="formula",all.y=F,all.x=T,drop=T)

Meta_feature_list<-aggregate 
deconv_peaklistsearch<-deconv_peaklist
deconv_peaklistsearch$response<-deconv_peaklist$intensities
PC1_top_spec<-deconv_peaklistsearch[,c("m.z","response")]
  PC1_top_spec$intensities<-round(PC1_top_spec$response/max(PC1_top_spec$response),digits = 8)
  finalsearchlist<-Meta_feature_list['&'(Meta_feature_list$mz<=max(PC1_top_spec$m.z),Meta_feature_list$mz>=min(PC1_top_spec$m.z)),]
  searchmz<-NULL
  searchmz$mz<-unique(finalsearchlist$mz)
  
  
  PMFsum_para_obmz<-function(pimmzlist,spectrumlist,ppm){
    lowmz<-pimmzlist-pimmzlist*ppm/1000000
    highmz<-pimmzlist+pimmzlist*ppm/1000000
    return(sum(spectrumlist[data.table::between(spectrumlist[,"m.z"], lowmz, highmz,incbounds=T),2]))
  }
  if (length(searchmz$mz)>=500){
   searchhit<-bplapply(searchmz$mz,PMFsum_para_obmz,PC1_top_spec,ppm=ppm,BPPARAM = BPPARAM) 
  }else{
   searchhit<-bplapply(searchmz$mz,PMFsum_para_obmz,PC1_top_spec,ppm=ppm,BPPARAM = SerialParam()) 
  }
  
  searchhit<-unlist(searchhit)
  searchmz$intensity<-searchhit
  searchmz<-as.data.frame(searchmz)
  searchmz<-searchmz[searchmz$intensity!=0,]
  
  searchmerge<-merge(searchmz,finalsearchlist,by="mz")
  searchmerge.pathway.metabolite<-unique(searchmerge[,c("mz","Metabolite.Name")])
  searchmerge.metabolite<-unique(searchmerge[,c("mz","Metabolite.Name","formula","adduct","intensity")])
  library(tidyverse)
  #searchmerge.metabolite.table<-searchmerge.metabolite %>% group_by_at("formula") %>% summarize_all(unique) %>% 
  #summarize_all(funs(paste0(unique(.[!is.na(.)]), collapse= "@")))
  searchmerge.metabolite.table<-searchmerge.metabolite %>% group_by_at(vars(formula)) %>% 
  summarize_all(funs(paste0(unique(.[!is.na(.)]), collapse= "@")))
  searchmerge.metabolite.table$intensity<-as.numeric(searchmerge.metabolite.table$intensity)
  searchmerge.metabolite.table$Metabolite.Name<-unlist(lapply(searchmerge.metabolite.table$Metabolite.Name,function(x){
    str_split(x,"@")[[1]][1]}
    ))
  searchmerge.metabolite.table_int<-searchmerge.metabolite.table[abs(searchmerge.metabolite.table$intensity)>signal_thres,]
  searchmerge.metabolite.table$mz<-as.numeric(searchmerge.metabolite.table$mz)
return(searchmerge.metabolite.table)
}

load_pixel_label<-function(combinedimdata,datafile_base,workdir,coordata_file="coordata.csv",pixel_idx_col=base::row.names,label_col="pattern",...){
  library(Cardinal)
  library(stringr)
  library(HiTMaP)
  library(stringr)
...
For detailed installation guide of MetaboAnalystR
...
#change your own project directory here
project_dir <- "R:/MVL/Work Folders/Dingchang/Manuscript/test/"
large_files_dir <- "R:/MVL/Work Folders/Dingchang/Manuscript/test/large_files/"
